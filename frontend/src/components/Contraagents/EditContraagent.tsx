...
import { type ApiError, type ContraagentUpdate, ContraagentsService } from "../../client"
import useCustomToast from "../../hooks/useCustomToast"
import { handleError } from "../../utils"

interface EditContraagentProps {
  isOpen: boolean
  onClose: () => void
  contraagent: any
}

interface ContraagentFormData extends ContraagentUpdate {
  validateVat?: boolean
}

const EditContraagent = ({ isOpen, onClose, contraagent }: EditContraagentProps) => {
  ...
  const {
    ...
    criteriaMode: "all",
    defaultValues: {
      name: contraagent?.name || "",
      email: contraagent?.email || "",
      phone: contraagent?.phone || "",
      website: contraagent?.website || "",
      fax: contraagent?.fax || "",
      is_company: contraagent?.is_company ?? true,
      is_customer: contraagent?.is_customer ?? false,
      is_supplier: contraagent?.is_supplier ?? false,
      is_active: contraagent?.is_active ?? true,
      registration_number: contraagent?.registration_number || "",
      vat_number: contraagent?.vat_number || "",
      street_name: contraagent?.street_name || "",
      building_number: contraagent?.building_number || "",
      building: contraagent?.building || "",
      postal_code: contraagent?.postal_code || "",
      city: contraagent?.city || "",
      region: contraagent?.region || "",
      country: contraagent?.country || "BG",
      additional_address_detail: contraagent?.additional_address_detail || "",
      contact_person_title: contraagent?.contact_person_title || "",
      contact_person_first_name: contraagent?.contact_person_first_name || "",
      contact_person_last_name: contraagent?.contact_person_last_name || "",
      tax_type: contraagent?.tax_type || "",
      tax_authority: contraagent?.tax_authority || "",
      self_billing_indicator: contraagent?.self_billing_indicator ?? false,
      related_party: contraagent?.related_party ?? false,
      iban_number: contraagent?.iban_number || "",
      bank_account_number: contraagent?.bank_account_number || "",
      bank_sort_code: contraagent?.bank_sort_code || "",
      opening_debit_balance: contraagent?.opening_debit_balance || 0,
      opening_credit_balance: contraagent?.opening_credit_balance || 0,
      name_latin: contraagent?.name_latin || "",
      name_cyrillic: contraagent?.name_cyrillic || "",
      notes: contraagent?.notes || "",
      validateVat: true,
    },
  })

  // Reset form when contraagent changes
  useEffect(() => {
    if (contraagent) {
      reset({
        name: contraagent.name || "",
    ...
    <>
      ...
      >
        ...
        <ModalContent as="form" onSubmit={handleSubmit(onSubmit)}>
          <ModalHeader>Edit Contraagent</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
...
export default EditContraagent